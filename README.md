...
- **环境检测**：一键检测 npm / node / dsh 已装版本与 npm 最新版本（断网时友好提示）
- **运行状态**：三重确认（端口监听 + HTTP 应答 + 进程特征），绿/红/琥珀状态灯一目了然
- **完整控制**：一键 **启动 / 停止 / 重启** dsh web，打开浏览器访问界面
- **更新支持**：检查更新 + 一键升级 `@deepseek-ai/dsh`（日志实时回显）
- **托盘驻留**：`—` 最小化到任务栏，`✕` 最小化到托盘；托盘菜单一键操作
- **设置面板**：端口、启动后自动开浏览器、关闭最小化到托盘、启动时自动检测（配置持久化，可扩展）
- **单实例**：重复打开只聚焦已有窗口
- **零依赖**：单文件 exe（约 70KB），无需安装任何运行时 / SDK / npm 包
...
A：未签名 exe 的正常提示。点击「更多信息 → 仍要运行」。介意的话可自行用代码签名证书签名后分发。

**Q：提示找不到 dsh？**
A：先点「更新 dsh」安装，或确认 npm 环境正常（`npm --version` 有输出）。
